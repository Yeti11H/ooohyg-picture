...
import com.h.ooohygpicture.common.DeleteRequest;
...
import com.h.ooohygpicture.model.dto.VipExchangeRequest;
import com.h.ooohygpicture.model.dto.user.*;
...
public class UserController {
    ...
    public BaseResponse<Boolean> signIn(HttpServletRequest request) {
        ...
        boolean result= userService.updateCredits(user.getId(),10L);
        if (result) {
            // 3. 记流水 (这一步其实应该和上面放在同一个事务里，明天我们再优化)
            CreditLog creditLog = new CreditLog();
            creditLog.setUserId(user.getId());
            creditLog.setAmount(10L);
            creditLog.setType(0); // 0-签到
            creditLog.setRemark("每日签到");
